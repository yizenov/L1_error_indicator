...
2. Run the following command: /usr/bin/python3 scripts/L1_error_job_greedy.py arg1\n \
...
if len(sys.argv) != 2:
    ...
    try:
        ...
        JOINS_TO_CONSIDER = 21
        LOGISTIC_GROWTH_RATE = -1.5
        avoid_queries = []  # [] no skipping
        ...
        if is_compass not in [0, 1]: 
            ...
            true_cost_data_file = "input_data/job/greedy_traversals-opt-cost.csv"
            if is_compass:
                cardinality_file, est_index = "input_data/job/results_estimates-JOB.csv", 4
                plans_file = "input_data/job/greedy_traversals-opt-cost-compass.csv"
                output_f_file = "input_data/job/L1-errors-agg-greedy-compass.csv"
            else:
                cardinality_file, est_index = "input_data/job/results_estimates-JOB.csv", 3
                plans_file = "input_data/job/greedy_traversals-opt-cost-psql.csv"
                output_f_file = "input_data/job/L1-errors-agg-greedy-psql.csv"
            ...
            for idx, file_name in enumerate(input_query_files):
                ...
                if int(query_name[:-1]) in avoid_queries: continue
                    ...
                    for join_size in range(2, len(table_list)):
                        ...
                        for sub in all_queries[query_name]:
                            if all_queries[query_name][sub] == join_size:
                                true_cards[sub] = all_true_cardinalities[query_name][sub]
                                est_cards[sub] = all_estimated_cardinalities[query_name][sub]
            ...
            for c_idx, complex in enumerate([[4, 9], [10, 19], [20, 28]]):
            ...
            print("better selected plans than true greedy optimal: " + str(len(better_than_optimal)))
