...
2. Run the following command: /usr/bin/python3 scripts/L1_error_job_exhaustive.py arg1\n \
...
if len(sys.argv) != 2:
    ...
    try:
        ...
        if is_compass not in [0, 1]: 
            ...
            true_cost_data_file = "input_data/job/exhaustive_traversals-opt-cost.csv"
            if is_compass:
                cardinality_file, est_index = "input_data/job/results_estimates-JOB.csv", 4
                plans_file = "input_data/job/exhaustive_traversals-opt-cost-compass.csv"
                output_f_file = "input_data/job/L1-errors-agg-exhaustive-compass.csv"
            else:
                cardinality_file, est_index = "input_data/job/results_estimates-JOB.csv", 3
                plans_file = "input_data/job/exhaustive_traversals-opt-cost-psql.csv"
                output_f_file = "input_data/job/L1-errors-agg-exhaustive-psql.csv"
            ...
            for query in subquery_plans:
                if int(query[:-1]) in avoid_queries: continue
