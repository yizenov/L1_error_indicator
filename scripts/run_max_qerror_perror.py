...
if len(sys.argv) != 1:
    ...
    try:
        ...
        output_f_file = "output_data/job/res_max_qerror_perror_exhaustive.csv"
        ...
        with open(cardinality_job_file, "r") as input_f:
            for idx, line in enumerate(input_f):
                ...
                true_cardinality = int(float(line.strip().split(",")[2]))
                psql_estimate = int(float(line.strip().split(",")[3]))
